...
TEST_DB_PATH = Path(__file__).parent / "test_db.json"


class TestFileUserTable(unittest.TestCase):
    def setUp(self):
        if TEST_DB_PATH.exists():
            TEST_DB_PATH.unlink()
        self.user_table = FileUserTable(TEST_DB_PATH)
        self.assertIsInstance(self.user_table, FileUserTable)
    ...
    def test_load_from_file(self):
        ...
        reloaded = FileUserTable(TEST_DB_PATH)
    ...
    def test_empty_file(self):
        ...
        table = FileUserTable(TEST_DB_PATH)
        self.assertEqual(table.select_record(), [])

    def test_missing_file(self):
        missing_path = Path(__file__).parent / "missing_db.json"
        table = FileUserTable(missing_path)
        self.assertEqual(table.select_record(), [])
    ...
    def test_invaluser_id_record_format(self):
        ...
        self.assertIn("Некорректная запись", str(context.exception))

    def test_read_error(self):
        dir_path = Path(__file__).parent / "test_db_dir"
        dir_path.mkdir(exist_ok=True)

        try:
            with self.assertRaises(errors.StorageReadError) as context:
                FileUserTable(dir_path)
            self.assertIn("Не удалось прочитать файл", str(context.exception))
        finally:
            dir_path.rmdir()
    ...
    def test_write_error(self):
        ...
        try:
            ...
            with self.assertRaises(errors.StorageWriteError) as context:
                self.user_table.create_record(1, "Ja", "Morant", 26, "+12")
            self.assertIn("Не удалось записать файл", str(context.exception))
        finally:
            dir_path.rmdir()
